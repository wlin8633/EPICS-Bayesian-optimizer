...
def process_analysis_file(file_path):
    ...
    try:
        ...
        with np.load(file_path, allow_pickle=True) as data:
            ...
            # The keys ('y1', 'y2') MUST match the column names in your Google Sheet.
            results = {
                'shot_number': shot_number,
                'Obj': - np.sum(data['mean_spectrum'] * data['energy_axis']) * np.diff(data['energy_axis']).mean(), 
            }
...
if __name__ == '__main__':
    ...
    # %%
    ds = np.load("sample_tp_analysis.npz", allow_pickle=True)
    ds.files
    ...
    ds['shot_number'].item()
    # %% find energy * counts
    weighted_spectrum = mean_spectrum * energy_axis
    obj = np.sum(weighted_spectrum)
    print(f"Objective value (sum of energy * counts): {obj}")
