...
def suggest_next_params_bo(
    ...
    if phase == "random":
        suggestion = _sample_uniform(inputNames)
        # If context is known, override the random context with the known value
        if current_context:
            for i, k in enumerate(keys):
                if k in current_context:
                    suggestion[i] = current_context[k]
        return suggestion
    ...
    # Clip history_x to exactly within the bounds
    for i, k in enumerate(keys):
        lo, hi = inputNames[k]
        history_x_arr[:, i] = np.clip(history_x_arr[:, i], lo, hi)
        
    history_obj = df_cleaned[optName].to_numpy(dtype=float)
    
    # BoTorch maximizes by default. If optType == "min", negate the objective
    if optType == "min":
        history_obj = -history_obj
        
    # Convert to PyTorch Tensors
    train_X = torch.tensor(history_x_arr, dtype=torch.double)
    train_Y = torch.tensor(history_obj, dtype=torch.double).unsqueeze(-1)
    
    bounds_tensor = torch.tensor(
        [[inputNames[k][0] for k in keys],
         [inputNames[k][1] for k in keys]], 
        dtype=torch.double
    )
    
    # Initialize GP Model with automatic normalization/standardization
    gp = SingleTaskGP(
        train_X, train_Y,
    ...
    if not context_keys:
        ...
        candidate, acq_value = optimize_acqf(
            EI, bounds=bounds_tensor, q=1, num_restarts=5, raw_samples=20
        )
        ...
        # Determine indices
        context_indices = [keys.index(k) for k in context_keys]
        action_indices = [i for i in range(len(keys)) if i not in context_indices]
        
        # If Unknown Context, we use the historical mean as the robust estimate
        if current_context is None:
            current_context = {k: float(train_X[:, keys.index(k)].mean()) for k in context_keys}
            
        # Prepare fixed features dict {index: value}
        fixed_features = {keys.index(k): current_context[k] for k in context_keys if k in current_context}
        
        # Wrap EI with FixedFeatureAcquisitionFunction
        fixed_EI = FixedFeatureAcquisitionFunction(
            acq_function=EI,
            d=len(keys),
        ...
        # Reduce bounds to only action dimensions
        action_bounds = bounds_tensor[:, action_indices]
        
        candidate_reduced, acq_value = optimize_acqf(
            fixed_EI, bounds=action_bounds, q=1, num_restarts=5, raw_samples=20
        )
        
        # Reconstruct the full D-dimensional suggestion list
        candidate_full = [0.0] * len(keys)
        for i, action_idx in enumerate(action_indices):
            candidate_full[action_idx] = candidate_reduced[0][i].item()
        for ctx_idx, val in fixed_features.items():
            candidate_full[ctx_idx] = val
